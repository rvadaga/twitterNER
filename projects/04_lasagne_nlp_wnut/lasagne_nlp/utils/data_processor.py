...
def read_conll_orthographic_sequence(path, word_alphabet):
    """
    read data from file in conll format
    :param path: file path
    :param word_alphabet: alphabet of words
...
def load_dataset_sequence_labeling(train_path, dev_path, test_path, word_column=1, label_column=4,
    ...
    def construct_orth_tensor_fine_tune(orth_word_index_sentences):
        X = np.empty([len(orth_word_index_sentences), max_length], dtype=np.int32)

        for i in range(len(orth_word_index_sentences)):
            word_ids = orth_word_index_sentences[i]
            length = len(word_ids)
            for j in range(length):
                wid = word_ids[j]
                X[i, j] = wid
    ...
    def generate_dataset_fine_tune():
        ...
        embedd_dict, embedd_dim, caseless = utils.load_word_embedding_dict(embedding, embedding_path, word_alphabet,
                                                                           logger)
        orth_word_embedding, orth_embedd_dim = utils.load_ortho_word_embedding_dict(word_orth_alphabet, logger)
        ...
        X_train_orth = construct_orth_tensor_fine_tune(orth_index_sentences_train)
        ...
        X_test_orth = construct_orth_tensor_fine_tune(orth_index_sentences_test)
        C_train, C_dev, C_test, char_embedd_table = generate_character_data(word_sentences_train, word_sentences_dev,
            ...
            None, None, None, None)
        orth_C_train, orth_C_dev, orth_C_test, orth_char_embedd_table = generate_character_data(orth_sentences_train, orth_sentences_dev,
                                                                            orth_sentences_test,
                                                                            max_length) if use_character else (
            None, None, None, None)
        return X_train, Y_train, mask_train, X_dev, Y_dev, mask_dev, X_test, Y_test, mask_test, \
    ...
    word_orth_alphabet = Alphabet('word_orth')
    character_orth = Alphabet('char_orth')
    ...
    orth_sentences_train, orth_index_sentences_train = read_conll_orthographic_sequence(train_path, word_orth_alphabet)

    # if oov is "random" and do not fine tune, close word_alphabet
    if oov == "random" and not fine_tune:
        logger.info("Close word alphabet.")
        word_alphabet.close()
        word_orth_alphabet.close()  # TODO: Is this necessary?
    ...
    orth_sentences_dev, orth_index_sentences_dev = read_conll_orthographic_sequence(dev_path, word_orth_alphabet)
    ...
    orth_sentences_test, orth_index_sentences_test = read_conll_orthographic_sequence(test_path, word_orth_alphabet)
