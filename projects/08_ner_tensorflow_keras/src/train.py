...
# Set up logging.
# Logger will output both to stdout and a file
logger = utils.get_logger("NeuralNet", LOG_FILE)
...
# This is passed so that the data is unbuffered, and
# is immediately pushed into the file
bufsize = 0
f = open(args.save_dir + "/results.txt", "a", bufsize)

# TODO: see if tabs can be introduced in results.txt
...
with tf.Graph().as_default():
    with tf.Session() as session:
        ...
        # # Add l2 regularisation to negative log likelihood
        # vars = tf.trainable_variables()
        # loss_l2 = tf.add_n([
        #     tf.nn.l2_loss(v) for v in vars if 'bias' not in v.name]) * 0.001
        # loss = loss_nll + loss_l2

        loss = loss_nll

        # Set the optimizer, optimize the loss function and clip the
        # gradients
        ...
        capped_gvs = [
            (tf.clip_by_value(grad, -args.grad_clip, args.grad_clip), var)
            for grad, var in gvs]
