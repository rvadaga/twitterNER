...
def parse_args():
    ...
    parser.add_argument('--oov',
                        ...
                        required=True)
    parser.add_argument('--orth_emb_dim',
                        type=int,
                        default=200,
...
def validate_args(args):
    ...
    if os.path.isdir(args.save_dir):
        ...
        os.makedirs(args.save_dir + "/dev_predictions")
        os.makedirs(args.save_dir + "/dev_scores")
...
def save_predictions(
        ...
        count,
        is_flattened):
    batch_size, max_sentence_len = targets.shape
        ...
        for i in range(batch_size):
            for j in range(max_sentence_len):
                if masks[i, j] > 0.:
                    ...
                    f.write(
                        "%s %s %s\n" %
                        (unicode(word_collection.get_instance(inputs[i, j]),
                                 "utf8"),
                         label_collection.get_instance(targets[i, j] + 1),
