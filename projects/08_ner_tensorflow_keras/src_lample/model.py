    ...
    with tf.variable_scope("char"):
        ...
        if not args.bilstm_bilstm:
            ...
            outputs, _ = tf.nn.bidirectional_dynamic_rnn(
                ...
                sequence_length=char_seq_length_input, dtype=tf.float32)
            fwd_emb = tf.slice(
                input_=outputs[0],
...
def get_feed_dict(data,
    ...
    if args.bilstm_bilstm:
        feed_dict[char_seq_length_input] = np.reshape(data["word_lengths"][batch_id*batch_size:(batch_id+1)*batch_size],
                                                      [-1])
