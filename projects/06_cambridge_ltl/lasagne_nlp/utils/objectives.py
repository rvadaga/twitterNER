...
def crf_loss(energies, targets, masks):
    ...
    :param energies: Theano 4D tensor
        energies of each step. the shape is
        [batch_size, n_time_steps, num_labels, num_labels],
    ...
    def inner_function(energies_one_step,
                       targets_one_step,
                       mask_one_step,
                       prior_partition,
                       prev_label,
        ...
        return [partition_t, targets_one_step,
                tg_energy +
                energies_one_step[T.arange(energies_one_step.shape[0]),
                                  prev_label,
                                  targets_one_step]]

    # Input should be provided as
    # (n_batch, n_time_steps, num_labels, num_labels)
    # but scan requires the iterable dimension to be first
    # So, we need to dimshuffle to
    # (n_time_steps, n_batch, num_labels, num_labels)
    ...
    initials = [energies_shuffled[0, :, -1, :],
                target_time0,
                energy_time0[T.arange(energy_time0.shape[0]),
                             init_label, target_time0]]
    [partitions, _, target_energies], _ = \
        theano.scan(fn=inner_function,
...
def crf_accuracy(energies, targets):
    ...
    # (n_time_steps, n_batch, num_labels, num_labels)
    energies_shuffled = energies.dimshuffle(1, 0, 2, 3)
    # pi at time 0 is the last rwo at time 0. but we need to remove the
    # last column which is the pad symbol.
    pi_time0 = energies_shuffled[0, :, -1, :-1]

    # the last row and column is the tag for pad symbol.
    # reduce these two dimensions by 1 to remove that.
    # now the shape of energies_shuffled is
    # [n_time_steps, b_batch, t, t] where t = num_labels - 1.
    ...
    prediction_revered = T.concatenate([pointer_n.dimshuffle(0, 'x'),
                                       back_pointers.dimshuffle(1, 0)],
                                       axis=1)
    prediction = prediction_revered[:,
        T.arange(prediction_revered.shape[1] - 1, -1, -1)]
