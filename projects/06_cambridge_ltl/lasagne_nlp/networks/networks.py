...
def build_BiLSTM(incoming,
                 ...
                 dropout=True,
                 in_to_out=False):
    # construct the forward and backward rnns. Now, Ws are initialized by
    # Glorot initializer with default arguments.
    ...
    lstm_forward = lasagne.layers.LSTMLayer(incoming,
                                            num_units,
                                            mask_input=mask,
    ...
    cell_backward = Gate(W_in=lasagne.init.GlorotUniform(),
                         ...
                         W_cell=None,
                         nonlinearity=nonlinearities.tanh)
    lstm_backward = lasagne.layers.LSTMLayer(incoming,
                                             num_units,
                                             mask_input=mask,
                                             grad_clipping=grad_clipping,
                                             nonlinearity=nonlinearities.tanh,
    ...
    concat = lasagne.layers.concat([lstm_forward, lstm_backward],
                                   axis=2,
                                   name="bi-lstm")
    ...
    # the shape of BiRNN output (concat) is
    # (batch_size, input_length, 2 * num_hidden_units)
    return concat


def build_BiLSTM_CNN(incoming1,
                     incoming2,
                     incoming3,
                     incoming4,
                     num_units,
    ...
    if dropout:
        ...
        incoming3 = lasagne.layers.DropoutLayer(incoming3, p=0.5)
    ...
    cnn_layer = lasagne.layers.Conv1DLayer(
        ...
        name='cnn')
    orth_cnn_layer = lasagne.layers.Conv1DLayer(
        incoming3,
        num_filters=num_filters,
        filter_size=conv_window,
        pad='full',
        nonlinearity=lasagne.nonlinearities.tanh,
        name='orth-cnn')
    # infer the pool size for pooling
    # (pool size should go through all time step of cnn)
    _, _, pool_size = cnn_layer.output_shape
    _, _, orth_pool_size = orth_cnn_layer.output_shape
    # construct max pool layer
    pool_layer = lasagne.layers.MaxPool1DLayer(cnn_layer,
                                               pool_size=pool_size)
    orth_pool_layer = lasagne.layers.MaxPool1DLayer(orth_cnn_layer,
                                                    pool_size=orth_pool_size)
    # reshape the layer to match lstm incoming
    # layer [batch * sent_length, num_filters, 1] -->
    #       [batch, sent_length, num_filters]
    output_cnn_layer = lasagne.layers.reshape(pool_layer,
                                              (-1, sent_length, [1]))
    orth_output_cnn_layer = lasagne.layers.reshape(orth_pool_layer,
                                              (-1, sent_length, [1]))

    # finally, concatenate the two incoming layers together.
    incoming = lasagne.layers.concat([output_cnn_layer,
                                      incoming2,
                                      orth_output_cnn_layer,
                                      incoming4], axis=2)

    return build_BiLSTM(incoming,
                        num_units,
                        mask=mask,
                        grad_clipping=grad_clipping,
                        peepholes=peepholes,
